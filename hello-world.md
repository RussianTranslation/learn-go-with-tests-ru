...
Если вы используете unix-подобную ОС и вам нравится подход Go с использованием `$GOPATH` (который является самым простым для настройки), то можете запустить команду `mkdir -p $GOPATH/src/github.com/$USER/hello`.

Для следующих разделов вы можете создать новую папку для кода с любым именем. Например, `$GOPATH/src/github.com/{your-user-id}/integers` может быть подходящей папкой для следующего раздела. Некоторым читателям может понравиться идея создать единую папку для всех работ, например, "learn-go-with-tests/hello". В общем, вам решать, как структурировать папки.
...
Когда вы пишите программу на Go, у вас будет пакет `main`, внутри которого будет определена функция `main`. Пакеты - это способ группирования связанного между собой Go-кода.
